...
def mysql_connection(host, user, passwd, database=None):
    ...
    return connection
resposta = str(sys.argv[1])
maquina = str(sys.argv[2])
cpuM = str(sys.argv[3])
memoriaM = str(sys.argv[4])
discoM = str(sys.argv[5])
# resposta = (input('Gostaria de começar o monitoramento? ("S" ou "N"): '))
if(resposta == "S" or resposta == "s"):
    #maquina = (input('Qual é o id do seu hardware?'))
    #cpuM = (input('Deseja monitorar a cpu? ("S" ou "N")'))
    #memoriaM = (input('Deseja monitorar a ram? ("S" ou "N")'))
    #discoM = (input('Deseja monitorar disco? ("S" ou "N")'))
